...
⚠️ **AND THE CONTRACT IS NARROWER THAN IT READS (2026-09-06 20:32 EDT).** Three of the detector's rules — `design-system-font`, `design-system-color`, `design-system-radius` — fire only when `DESIGN.md` declares a palette and a type stack in the format the parser reads. Ours has `## Type` and `## The colour rule`: real content, wrong shape, so `doctor.mjs` reports *"no colors, typography section"* and those rules check nothing. **Falsified live:** a file containing `#FF00FF`, `#7C3AED`, `border-radius:17px` and Comic Sans returns **0 findings**, inside the project, with `DESIGN.md` discoverable. Every "exactly one finding" recorded before this date is true about the rules that ran and silent about three that did not. Filed in `devoid-deferred-list.md`.
