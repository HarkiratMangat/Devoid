...
`docs/shots/the-question.webp` is recaptured (the mask, not the box). `09-seam.webp` is NOT restored: the underlying seam genuinely differs now, but the raw capture is uncropped app chrome with a hard-to-see hatch and no visual link to the numbers that prove it — the exact legibility failure this screenshot was pulled for the first time. Restoring it needs a real crop, not a bigger claim; filed as its own item. The README's *"instead of a hex code and a bounding box"* claim, removed when this was only half-true, is back.
