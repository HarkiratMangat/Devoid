...
**Stage 1 — engine layer.** `server/{validate,engine,concurrency,flags,cli,preview,render,assets}.py`. `build_parser()` was added to the **skill repo** (`/Applications/Claude Code/Gif-Background-Remover`, branch `feat/build-parser-factory`, commit `ecb5e2f`, **not merged to that repo's `main`, not pushed**) — Devoid's `server/flags.py` depends on that branch being checked out in the skill repo right now. ⚠️ **Checking that repo back to `main` breaks `GET /api/flags`** (loudly — `503 ParserFactoryMissing`, never a silent fallback). Land that PR in the skill repo, or this stays a standing dependency on an uncommitted branch state in a sibling repo.
...
**Stage 5 — memory.** `server/{labels,jobs,journal,appendlog}.py`, `web/advice.js` (now actually loaded — see the merge-bug list above). Concurrency proven both ways: 8 threads × 50 appends (same process) and 4 spawned processes × 50 appends each, both give exactly the expected line count with zero interleaved/corrupted JSON. The label-log pointer in the skill repo (`scripts/harness/labels/README.md`) was stale ("that file does not exist yet") and was corrected on a small branch there (`docs/fix-labels-pointer`, commit `3cde23b`, **also unmerged/unpushed in the skill repo**).
...
- **The `build_parser()` and labels-pointer PRs in the skill repo are uncommitted branches, not merged, not pushed.** `GET /api/flags` depends on that branch being checked out RIGHT NOW in that sibling repo. This is fragile — a `git checkout main` in the skill repo breaks Devoid's flag drawer with no warning until someone hits it.
