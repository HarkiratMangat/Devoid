...
### The gate was writing the corpus it exists to protect — 2026-09-07 01:32 EDT

`labels/protection.jsonl` is tracked evidence: the only labels anywhere for the protection decision, pointed at by path from the engine repo because nothing else collects them. Deleting the `Answer` button made picking a side submit on the spot — and `capture-window.mjs` picks sides. Twelve gate runs appended **24 synthetic rows**, twelve asset ids over **one** bbox, all `protect`, and they were committed before anyone looked.

**A corpus whose rows were generated by the instrument measuring it is worse than an empty one, because an empty one is honest.** `DEVOID_DATA_DIR` already existed for the packaged app; the gate now runs against `local/gate-data`, and it weighs the tracked file byte-for-byte across the run so the redirect cannot be quietly dropped later. The 24 rows are gone; the scratch copy holds 2, which is the proof the gate still answers.

⚠️ **The trigger was a behaviour change three commits away from the file it damaged.** Nothing in the diff of "delete the Answer button" mentions labels.

---

### An enabled hook that is wired to nothing, and a matcher that cannot see us — 2026-09-07 01:32 EDT

`/impeccable hooks on` reported `state: enabled` and, in the same breath, *"No installed provider skill folders found to repair"* — its installer only writes the manifest when the skill sits inside the project, and here it is installed at user level. **Enabled and reaching nobody**, which is this machine's most-repeated hook failure, recorded in `~/.claude/CLAUDE.md` twice already.

Writing `.claude/settings.local.json` by hand fixed it, and a **live** test proved it: a deliberately defective `.css` written with the Write tool came back with four findings. A pipe test would have proved nothing.

⚠️ **Then the second half.** The matcher is `Edit|Write`. This repo's edits are `python3` heredocs through Bash — the batching technique its own conventions ask for — so the hook is blind to almost everything a session here actually does, and the Stop deep pass inherits that blindness because it scans the files the per-edit pass marked. `npm run check:design` closes it from the other side by asking git what changed instead of asking the harness what it saw.
